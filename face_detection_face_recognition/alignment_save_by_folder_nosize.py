...
def face_preprocess(image, landmark_model_type = 'large', addsize = 0):
    ...
    for i, face_landmarks_item in enumerate(face_landmarks_dict):
        ...
        rotated_landmarks = rotate_landmarks(landmarks = face_landmarks_item,
                                            eye_center = eye_center, angle = angle, row = image.shape[0])
        # crop face according to landmarks
        cropped_face_item, left, top = corp_face(image_array = aligned_face, landmarks = rotated_landmarks, addsize = addsize)
        # transfer landmarks to fit the cropped face
        transferred_landmarks_item = transfer_landmark(landmarks = rotated_landmarks, left = left, top = top)
        # add to the end of list
        cropped_face.append(cropped_face_item)
        transferred_landmarks.append(transferred_landmarks_item)
...
def save_image(imglist, addsize = 0):
    ...
    for index, imgpath in enumerate(imglist):
        ...
        for i, face_item in enumerate(face):
            cv2.imwrite("./results/%s_family%d_face%d.jpg" % (family_name, index, i), face_item)
